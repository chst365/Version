...
<mapper namespace="com.shudailaoshi.dao.${moduleName}.${entityName}Dao">
	...
	<select id="list${entityName}" resultMap="${packageName}Map">
		...
		order by <#noparse>${</#noparse>com.shudailaoshi.utils.StringUtil@camelhumpToUnderline(${packageName}Query.sort.property)} <#noparse>${</#noparse>${packageName}Query.sort.direction}
		</if>
