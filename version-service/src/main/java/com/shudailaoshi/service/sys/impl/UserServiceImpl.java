...
public class UserServiceImpl extends BaseServiceImpl<User> implements UserService {
	...
	public void removeUser(long id) {
		if (this.userDao.getByPrimaryKey(id).getUserName().equals(Constant.ADMIN)) {
			throw new ControllerException(ExceptionEnum.ADMIN_NOT_ALLOW_DELETE);
		}
		try {
	...
	public void freeze(long id) {
		if (this.userDao.getByPrimaryKey(id).getUserName().equals(Constant.ADMIN)) {
			throw new ControllerException(ExceptionEnum.ADMIN_NOT_ALLOW_FREEZE);
		}
		User user = new User();
		user.setId(id);
		user.setModifyTime(DateUtil.getTime());
	...
	public void grant(long id, Set<Long> roleIds) {
		if (this.userDao.getByPrimaryKey(id).getUserName().equals(Constant.ADMIN)) {
			throw new ControllerException(ExceptionEnum.ADMIN_NOT_NEED_GRANT);
		}
		UserRole userRole = new UserRole();
		userRole.setUserId(id);
		this.userRoleDao.remove(userRole);
